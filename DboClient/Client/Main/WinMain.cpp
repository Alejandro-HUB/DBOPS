                     ...
                     int       nCmdShow)
{
	...
	if ( NULL == lpCmdLine && strcmp( lpCmdLine, "nomutex" ) > 0 )
	{
		...
		{
			::MessageBox(NULL, "DragonBallOnline Private Server is already opened !!!", "DragonBallOnline Private Server", MB_OK);
			return 1;
		}
	}
#endif
	...
	if(!IsLimitDirectory())
	{
		::MessageBox(NULL, "A directory name length must not exceed 200 letters", "DragonBallOnline Private Server", MB_OK);
		return 1;
	}
